...
(async () => {
    ...
    if (argv['create-keys']) {
        ...
        return oadacerts.keys
            .create()
            .then((result) => {
                console.log(
                    'Keys created, converting to PEM and writing output'
                );
                return Promise.all([
                    fsp.writeFileAsync(
                        './public_key.pem',
                        pemjwk.jwk2pem(result.public)
                    ),
                    fsp.writeFileAsync(
                        './public_key.jwk',
                        JSON.stringify(result.public)
                    ),
                    fsp.writeFileAsync(
                        './private_key.pem',
                        pemjwk.jwk2pem(result.private)
                    ),
                    fsp.writeFileAsync(
                        './private_key.jwk',
                        JSON.stringify(result.private)
                    ),
                ]);
            })
            .then((result) => {

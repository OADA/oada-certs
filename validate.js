...
module.exports = function (sig, options) {
  return Promise.try(() => {
    ...
    options.trustedListCacheTime =
      typeof options.trustedListCacheTime === 'number'
        ? options.trustedListCacheTime
        : 3600; // Seconds
    options.additionalTrustedListURIs = options.additionalTrustedListURIs || [];
    options.disableDefaultTrustedListURI =
      options.disableDefaultTrustedListURI || false;

    // Build the list of all the trusted lists we're going to check
    let trustedListURIs = options.disableDefaultTrustedListURI
      ? []
      : [TRUSTED_LIST_URI];
    ...
    trace(
      'Starting trusted lists cache check, trustedListCache = ',
      trustedListCache
    );
    const now = Date.now() / 1000; // Convert ms to sec
    return Promise.all(
      trustedListURIs.map((listURI) => {
        if (
          !trustedListCache[listURI] ||
          trustedListCache[listURI].timeLastFetched <
            now - options.trustedListCacheTime
        ) {
          // Either not cached, or cache is old
          trace(
            'listURI ',
            listURI,
            ' is not in cache or is stale, fetching...'
          );
          return request
            .get(listURI)
            ...
            .then((result) => {
              ...
              trustedListCache[listURI] = newCacheObject;
              trace(
                'Fetched list from URI ',
                listURI,
                ', putting this into the cache: ',
                newCacheObject
              );
        ...
        trace(
          'listURI ',
          listURI,
          ' is in cache, returning cached value: ',
          trustedListCache[listURI]
        );
    ...
    .then((lists) => {
      ...
      if (!decoded || !decoded.header) {
        trace(
          'decoded signature is null or has no header. decoded = ',
          decoded
        );
      ...
      const foundList = lists.reduce((alreadyFound, l) => {
        ...
        if (l.body.version === '2') {
          ...
          if (typeof l.body.jkus.find === 'function') {
            ...
            foundJKU = l.body.jkus.find(
              (index) =>
                typeof index === 'string' &&
                index.length > 0 &&
                index === decoded.header.jku
            );
          }
          ...
          if (
            jwku.isJWKset(l.body.jwks) && // Search through the trusted JWKS set
            jwku.findJWK(
              decoded.header.jwk && decoded.header.jwk.kid,
              l.body.jwks
            )
          ) {
          ...
          trace(
            `Searched list ${l.listURI} for jwk or jku from header, foundJKU = `,
            foundJKU,
            ', foundJWKInJWKS = ',
            foundJWKInJWKS
          );
        ...
        return l.body.find((index) => index === decoded.header.jku); // Returns jku string
      ...
      return Promise.props({
        ...
        trusted: Boolean(foundList), // True/false whether trusted/not_trusted
        jwk: jwku
          .jwkForSignature(sig, foundList ? foundList : false, {
            timeout: options.timeout,
          })
          .catch((error) => {
            details.push({
    ...
    .then(async ({ decoded, trusted, jwk, details }) => {
      ...
      return Promise.props(returnValue);
    })
    ...
    .catch((error) => {
      info('Uncaught error in oadacerts.validate. err was: ', error);
      details.push({
        message: `Uncaught error in oadacerts.validate.  err was: ${JSON.stringify(
          error,
          false,
          '  '
        )}`,
